...
from numba.types import abstract
...
class R_Acot(FIFOQueue):
    #Ordenado por acotacion (segun el coste)
...
class PriorityQueue(Queue):
    ...
    def __init__(self, order=min, f=lambda x: x):
        update(self, A=[], order=order, f=f)    #Se actualiza la cola a vacia

    def append(self, item):
        bisect.insort(self.A, (self.f(item), item)) #Se ordena segun el valor (min o max)

    def __len__(self):
        return len(self.A) #Devuelve el tam

    def pop(self):
        if self.order == min:
            return self.A.pop(0)[1] #Si el valor es min, se devuelve el primer elemt
        else:
            return self.A.pop()[1]  #Si el valor es max, se devuelve el ultimo elemt
